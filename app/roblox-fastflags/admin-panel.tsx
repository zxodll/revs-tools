...
const ADMIN_CONFIG = {
  ...
  PASSWORD_HASH: "admin123", // This should be a proper hash in production
...
// Available platforms
const AVAILABLE_PLATFORMS = ["Roblox", "Roblox Studio", "Roblox Mobile", "Windows", "macOS", "Linux", "iOS", "Android"]
...
export function AdminPanel({ isOpen, onClose, presets, onPresetsUpdate, showToast }: AdminPanelProps) {
  ...
  const handleLogin = () => {
    if (isLockedOut) {
      ...
      showToast("Account Locked", `Too many failed attempts. Try again in ${remainingTime} minutes.`, "destructive")
      return
    }
    ...
    if (password === ADMIN_CONFIG.PASSWORD_HASH) {
      ...
      setSessionEndTime(Date.now() + ADMIN_CONFIG.SESSION_TIMEOUT)
      setLoginAttempts(0)
      setPassword("")
      showToast("Login Successful", "Welcome to the admin panel.")
    } else {
      ...
      if (newAttempts >= ADMIN_CONFIG.MAX_LOGIN_ATTEMPTS) {
        setIsLockedOut(true)
        setLockoutEndTime(Date.now() + ADMIN_CONFIG.LOCKOUT_DURATION)
        showToast("Account Locked", `Too many failed attempts. Account locked for 15 minutes.`, "destructive")
      } else {
        const remainingAttempts = ADMIN_CONFIG.MAX_LOGIN_ATTEMPTS - newAttempts
        showToast("Invalid Password", `${remainingAttempts} attempts remaining.`, "destructive")
      }
      setPassword("")
    }
  }
